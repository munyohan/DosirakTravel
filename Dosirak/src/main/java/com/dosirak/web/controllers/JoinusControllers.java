...
public class JoinusControllers {
	...
	public String login(String joinok, Model model) {
		System.out.println("midZZZZ");
		System.out.println("joinok : "+ joinok);
		model.addAttribute("joinok", joinok);
		return "login.jsp";
	...
	public String login(String mid, String pwd, HttpServletRequest request, HttpServletResponse response, Model model) {
		 ...
		 * (String)session.getAttribute("mid"); model.addAttribute("mid", mid);
		 */
	...
	public String join(Player player, HttpServletRequest request) {


		System.out.println("mid : " + player.getMid());
		System.out.println("pwd : " + player.getPwd());
		System.out.println("nickname : " + player.getNickname());
		int af = plDao.addPlayer(player);

		if (af == 1) {
			System.out.println("회원가입 성공");
			return "redirect:login.do?joinok=1";
		} else {
